...
# -*- coding: utf-8 -*-
"""
Module1_Main_CompleteをShift_JISに変換
"""
...
if __name__ == "__main__":
    ...
    input_path = os.path.join(vba_dir, "Module1_Main_Complete.bas")
    output_path = os.path.join(vba_dir, "Module1_Main_Complete_SJIS.bas")

    print("=== Module1_Main_Complete 変換 (UTF-8 → Shift_JIS) ===")
    print(f"対象ディレクトリ: {vba_dir}")
    print()

    if os.path.exists(input_path):
        print(f"変換中: Module1_Main_Complete.bas")
        convert_to_sjis(input_path, output_path)
        print()
    else:
        print(f"[ERROR] ファイルが見つかりません: Module1_Main_Complete.bas")
