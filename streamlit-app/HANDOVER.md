...
| バージョン | 日付 | 主な変更点 |
|-----------|------|-----------|
| v7.10 | 2025-12-12 | **Excelパース部門認識バグ修正**: `parse_uploaded_excel()`の部門シート判定条件に`部門`を追加。エクスポートされたExcelの`部門_XXX`形式シートが正しく認識されず、部門名が「派遣会社」等になる問題を修正。`部門_男性`→`男性`のように正しく部門名を抽出 |
